...
class SessionSum {
public:
  void UpdateResolving(const std::string& url,
                       int32_t dur) {
    boost::shared_ptr<Summary>& sum = GetSummary(url);
    sum->UpdateResolving(dur);
    sum->AddResolveRec(dur);
    _sumall.UpdateResolving(dur);
  }

  void UpdateConnecting(const std::string& url,
                        int32_t dur) {
    boost::shared_ptr<Summary>& sum = GetSummary(url);
    sum->UpdateConnecting(dur);
    sum->AddConnectRec(dur);
    _sumall.UpdateConnecting(dur);
  }

  void UpdateRecvHeader(const std::string& url,
                        int32_t dur) {
    boost::shared_ptr<Summary>& sum = GetSummary(url);
    sum->UpdateRecvHeader(dur);
    sum->AddRecvHeaderRec(dur);
    _sumall.UpdateRecvHeader(dur);
  }

  void UpdateFirstChunk(const std::string& url,
                        int32_t dur) {
    boost::shared_ptr<Summary>& sum = GetSummary(url);
    sum->UpdateFirstChunk(dur);
    sum->Add1stChunkRec(dur);
    _sumall.UpdateFirstChunk(dur);
  }

  void UpdateBytesPerSec(const std::string& url,
                         size_t bytes,
                         int32_t dur) {
    GetSummary(url)->UpdateKBytesPerSec(bytes, dur);
    _sumall.UpdateKBytesPerSec(bytes, dur);
  }

  void UpdateError(const std::string& url,
                   uint32_t err) {
    GetSummary(url)->UpdateError(err);
    _sumall.UpdateError(err);
  }
...
protected:
  ...
  struct Summary {
    ...
    void AddResolveRec(int32_t dur) {
      _resolve.AddValue(dur);
    }

    void AddConnectRec(int32_t dur) {
      _connect.AddValue(dur);
    }

    void AddRecvHeaderRec(int32_t dur) {
      _recvhdr.AddValue(dur);
    }

    void Add1stChunkRec(int32_t dur) {
...
public:
  ...
  TestArena(const boost::shared_ptr<SessionSum>& sum)
    : _sum(sum) {
  }

  virtual void OnResolved(PlaySession* sess,
                          int32_t dur) {
    std::string url = sess->GetPlayURL();
    _sum->UpdateResolving(url, dur);
  }

  virtual void OnConnected(PlaySession* sess,
                           int32_t dur) {
    std::string url = sess->GetPlayURL();
    _sum->UpdateConnecting(url, dur);
  }

  virtual void OnRecvHeader(PlaySession* sess,
                            int32_t dur) {
    std::string url = sess->GetPlayURL();
    _sum->UpdateRecvHeader(url, dur);
  }

  virtual void OnFirstChunk(PlaySession* sess,
                            int32_t dur) {
    std::string url = sess->GetPlayURL();
    _sum->UpdateFirstChunk(url, dur);
  }
  ...
  virtual void OnContent(PlaySession* sess,
                         ...
                         int32_t dur_in_ms) {
    std::string url = sess->GetPlayURL();
    _sum->UpdateBytesPerSec(url, bytes, dur_in_ms);
  }
...
protected:
  ...
  PlaySession* CreateSession(const std::string& u) {
    ...
    if (url.protocol() == "rtmp") {
      ...
      return new HTTPPlaySession(this, _ioServ, url, _cfg.Timeout());
    }
    return NULL;
  }
...
private:
  ...
  boost::shared_ptr<SessionSum> _sum;
  io_service _ioServ;
  TestConfig _cfg;
};
